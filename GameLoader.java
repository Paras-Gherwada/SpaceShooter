...
public class GameLoader extends Application {
    ...
    int old_points = 0, new_points = 0, death_count = 0;
    ...
    public void start(Stage stage) throws Exception {
        ...
        Text old_score = new Text(
            10, 
            20, 
            "Old Score : " + old_points
        );
        
        Text new_score = new Text(
            10,
            40,
            "New Score : " + new_points
        );
        ...
        AnimationTimer timer = new AnimationTimer() {
            public void handle(long now) {
                try {
                    if(pressedKeys.getOrDefault(KeyCode.A, false)) ps.turnLeft();
                    if(pressedKeys.getOrDefault(KeyCode.D, false)) ps.turnRight();
                    if(pressedKeys.getOrDefault(KeyCode.W, false)) ps.accelerate(0.05);
                    if(pressedKeys.getOrDefault(KeyCode.S, false)) ps.decelerate();
                    if(pressedKeys.getOrDefault(KeyCode.R, false)) ps.resetPosition((WIDTH/2)-25, (HEIGHT/2)-50);
                    scene.setOnKeyPressed(
                        e->{
                            pressedKeys.put(e.getCode(), true);
                            if(pressedKeys.getOrDefault(KeyCode.SPACE, false) && !end && !pause) {
                                try {
                                    Bullet b = new Bullet(
                                        ps.getObject().getTranslateX(), 
                                        ps.getObject().getTranslateY()
                                    );
